...
BuckleScript already maps basic OCaml primitive types into JS primitive types, 
...
Note here by default `Ai` is mapped into `i`, but we can customie it 
as follows:
[source,ocaml]
--------------
type t = 
...
Note the above derived js type is transparent: `int`, if we want to 
...
In this case, it would generate two functions of such type, note `newType` means
...
The `bs.deriving` also applies to the signature file, so that users don't have to 
...
Note here by default `Ai` is mapped into `"Ai"`, but we can customie it 
as follows:
[source,ocaml]
--------------
type t = [
...
Note the above derived js type is transparent: `string`, if we want to 
provide more type safety, we can annotate it as below:
...
Note the above derived js type is transparent, if we want to 
provide more type safety, we can annotate it as below:
...
----------
 ...
 var List = require("bs-platform-fix/lib/js/list");
