...
`sh   cd ./jscomp   npm_package_name=bs-platform-fix make world   export PATH=$(cwd)/jscomp/bin:$PATH`

This install the compiler `bsc.exe` into `./jscomp/bin` and adds it to your
current command path.


// `sh   BS_RELEASE_BUILD=1 npm_package_name=bs-platform-fix make world`
