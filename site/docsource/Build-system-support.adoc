...
The build system is installed as `bsb.exe` in `bs-platform-fix/bin/bsb.exe`. Due to a known issue in npm,
...
-----------
<1> Here we list `bs-platform-fix` as a peer dependency so that different repos shares the same compiler.
...
-----------
{
    ...
    "peerDependencies" : { "bs-platform-fix" : "^1.7.0" } //<2>
}
-----------
<1> List `bucklescript-tea` as dependency
<2> List `bs-platform-fix` as peer dependency
